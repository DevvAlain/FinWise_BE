...
const enforceWalletQuota = async (req, res, next) => {
    try {
        ...
        const count = await Wallet.countDocuments({ user: req.user.id, isActive: true });
        if (typeof plan.maxWallets === 'number' && count >= plan.maxWallets) {
            return res.status(403).json({ success: false, message: 'Vượt giới hạn số lượng ví theo gói' });
        }
        next();
    } catch (e) {
        console.error('Wallet quota error:', e);
        return res.status(500).json({ success: false, message: 'Lỗi kiểm tra hạn mức ví' });
    }
};
...
const enforceTransactionQuota = async (req, res, next) => {
    try {
        ...
        if (typeof plan.maxMonthlyTransactions === 'number' && usage.transactionsCount >= plan.maxMonthlyTransactions) {
            return res.status(403).json({ success: false, message: 'Vượt giới hạn số giao dịch theo gói tháng này' });
        }
        next();
    } catch (e) {
        console.error('Transaction quota error:', e);
        return res.status(500).json({ success: false, message: 'Lỗi kiểm tra hạn mức giao dịch' });
    }
};
