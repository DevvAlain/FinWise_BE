...
const updateMine = async (req, res) => {
    try {
        const result = await categoryService.updateMine(req.user.id, req.params.categoryId, req.body);
        return res.status(result.statusCode).json(result);
    } catch (error) {
...
const deleteMine = async (req, res) => {
    try {
        const result = await categoryService.deleteMine(req.user.id, req.params.categoryId);
        return res.status(result.statusCode).json(result);
    } catch (error) {
