...
const createSystemCategory = async (payload) => {
  ...
  if (!name)
    return { success: false, statusCode: 400, message: 'Thiếu tên danh mục' };
...
const deleteSystemCategory = async (categoryId) => {
  const cat = await ExpenseCategory.findOneAndDelete({
    _id: categoryId,
    isSystem: true,
  });
  if (!cat)
  ...
  return {
    success: true,
    statusCode: 200,
    message: 'Đã xóa danh mục hệ thống',
    item: cat,
  };
};
