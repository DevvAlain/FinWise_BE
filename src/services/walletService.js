...
const createWallet = async (userId, payload) => {
    ...
    if (!walletName || !walletType) {
        return { success: false, statusCode: 400, message: "Thiếu walletName hoặc walletType" };
    }
...
const listWallets = async (userId) => {
    const items = await Wallet.find({ user: userId, isActive: true }).sort({ createdAt: -1 });
...
const getWallet = async (userId, walletId) => {
    ...
    if (!wallet) return { success: false, statusCode: 404, message: "Không tìm thấy ví" };
    return { success: true, statusCode: 200, wallet };
};

const updateWallet = async (userId, walletId, payload) => {
    const allowed = ["walletName", "walletType", "currency", "provider", "accountNumber", "balance"];
    const updates = {};
...
export default { createWallet, listWallets, getWallet, updateWallet, deleteWallet };
