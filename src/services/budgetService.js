...
const getPeriodDates = (period, startDate = new Date()) => {
    ...
    switch (period) {
        ...
        case 'weekly':
            ...
            periodStart = new Date(date.getTime() - daysToMonday * 24 * 60 * 60 * 1000);
...
const updateQuotaUsage = async (userId, planId, periodMonth) => {
    try {
        ...
        if (!quotaUsage) {
            quotaUsage = await QuotaUsage.create({ user: userId, plan: planId, periodMonth });
        }
...
export const create = async (userId, budgetData) => {
    try {
        ...
        const { periodStart: calculatedStart, periodEnd } = getPeriodDates(period, periodStart);
        ...
        if (existingBudget) {
            throw new Error('Đã tồn tại ngân sách cho danh mục/ví này trong khoảng thời gian này');
        }
        ...
        // Update quota usage
        const activeSubscription = await Subscription.findOne({ user: userId, status: 'active' }).populate('plan');
...
export const list = async (userId, filters = {}) => {
    try {
        const { period, category, wallet, isActive, page = 1, limit = 10 } = filters;
